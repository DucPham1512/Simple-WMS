...
{
    private readonly DonViTinhPageModel _vm;
    ...
    private async void DonViTinh_CurrentCellEndEdit(object sender, DataGridCurrentCellEndEditEventArgs e)
    {
        ...
        if (mapping is not ("Ten_Don_Vi_Tinh" or "Ghi_Chu"))
            return;

        if (dataGrid.CurrentRow is not DonViTinh row)
            return;

        try
        {
            if (BindingContext is DonViTinhPageModel vm)
            {
